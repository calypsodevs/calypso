...
CzCLPWallet::CzCLPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zclp seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCLPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCLPSeed_deprecated()) {
                    LogPrintf("%s: Updated zCLP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zclp seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zclp wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCLPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCLPCount(nCountLastUsed);
    else if (!walletdb.ReadZCLPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCLPWallet::Lock()
{
    seedMaster = 0;
}

void CzCLPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCLPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCLP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCLPWallet::LoadMintPoolFromDB()
{
...
void CzCLPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCLPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCLPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zclpTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCLPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCLP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zclpTracker which also adds to database
    pwalletMain->zclpTracker->Add(dMint, true);
...
void CzCLPWallet::SeedToZCLP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCLPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCLPWallet::GenerateDeterministicZCLP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCLPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCLPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
