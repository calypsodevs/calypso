...
#include "config/calypso-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "calypso.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "calypsod.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the CLP and zCLP money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 42388, 42389));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CLP/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CLP/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, calypso, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CLP/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all CALYPSO specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-clpstake=<n>", strprintf(_("Enable or disable staking functionality for CLP inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zclpstake=<n>", strprintf(_("Enable or disable staking functionality for zCLP inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:42388"));
    ...
    strUsage += HelpMessageOpt("-backupzclp=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCLP minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zclpbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zCLP backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizecalypsoamount=<n>     " + strprintf(_("Keep N CLP anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The CALYPSO Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that CALYPSO is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CALYPSO Core is shutting down."));
...
#endif
    // Make sure only a single CALYPSO process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CALYPSO Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //CALYPSO: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //CALYPSO specific: zerocoin and spork DB's
                ...
                // CALYPSO: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZCLPMinted();
                        RecalculateZCLPSpent();
                    }
                    RecalculateCLPSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // CALYPSO: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CALYPSO Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart CALYPSO Core to complete") << "\n";
        ...
        zwalletMain = new CzCLPWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zCLPWallet
        uiInterface.InitMessage(_("Syncing zCLP wallet..."));

        bool fEnableZClpBackups = GetBoolArg("-backupzclp", true);
        pwalletMain->setZClpAutoBackups(fEnableZClpBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zclpTracker->Init();
...
//    }
//
//    nAnonymizeCalypsoAmount = GetArg("-anonymizecalypsoamount", 0);
//    if (nAnonymizeCalypsoAmount > 999999) nAnonymizeCalypsoAmount = 999999;
//    if (nAnonymizeCalypsoAmount < 2) nAnonymizeCalypsoAmount = 2;
    ...
    LogPrintf("Anonymize CALYPSO Amount %d\n", nAnonymizeCalypsoAmount);
    ...
    /* Denominations
       ...
       For example:
       1CLP+1000 == (.1CLP+100)*10
       10CLP+10000 == (1CLP+1000)*10
    */
