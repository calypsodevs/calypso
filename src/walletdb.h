...
public:
    ...
    bool WriteZCLPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCLPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCLPSeed_deprecated(uint256& seed);
    bool EraseZCLPSeed();
    bool EraseZCLPSeed_deprecated();

    bool WriteZCLPCount(const uint32_t& nCount);
    bool ReadZCLPCount(uint32_t& nCount);
