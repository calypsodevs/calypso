...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZClpStake(spend));
    } else {
        ...
        CClpStake* clpInput = new CClpStake();
        clpInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(clpInput);
    }
