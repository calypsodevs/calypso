...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CLP);
    ...
    if (!settings.contains("nAnonymizeCalypsoAmount"))
        settings.setValue("nAnonymizeCalypsoAmount", 1000);

    nAnonymizeCalypsoAmount = settings.value("nAnonymizeCalypsoAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCalypsoAmount"))
        SoftSetArg("-anonymizecalypsoamount", settings.value("nAnonymizeCalypsoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in calypso.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCalypsoAmount:
            return QVariant(nAnonymizeCalypsoAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCalypsoAmount:
            nAnonymizeCalypsoAmount = value.toInt();
            settings.setValue("nAnonymizeCalypsoAmount", nAnonymizeCalypsoAmount);
            emit anonymizeCalypsoAmountChanged(nAnonymizeCalypsoAmount);
            break;
        case CoinControlFeatures:
