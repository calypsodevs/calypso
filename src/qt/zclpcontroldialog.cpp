...
#include "ui_zclpcontroldialog.h"
...
std::set<std::string> ZClpControlDialog::setSelectedMints;
std::set<CMintMeta> ZClpControlDialog::setMints;

ZClpControlDialog::ZClpControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZClpControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZClpControlDialog::~ZClpControlDialog()
{
    delete ui;
}

void ZClpControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZClpControlDialog::updateList()
{
...
void ZClpControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZClpControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZClp_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZClpControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZClpControlDialog::GetSelectedMints()
{
...
void ZClpControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
