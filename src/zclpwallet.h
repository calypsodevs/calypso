...
#ifndef CALYPSO_ZCLPWALLET_H
#define CALYPSO_ZCLPWALLET_H
...
public:
    CzCLPWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCLP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCLP(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //CALYPSO_ZCLPWALLET_H
